...
#include "Roguelike/RoguelikeExitTrigger.h"
...
void ARoguelikeExitTrigger::ActivateExit()
{
	...
	if (TriggerBox && !bHasTriggered)
	{
		...
		TriggerBox->GetOverlappingActors(OverlappingPlayers, AHikariPlayerCharacter::StaticClass());
		for (AActor* Actor : OverlappingPlayers)
		{
			if (AHikariPlayerCharacter* Player = Cast<AHikariPlayerCharacter>(Actor))
			{
				bHasTriggered = true;
...
)
{
	...
	AHikariPlayerCharacter* Player = Cast<AHikariPlayerCharacter>(OtherActor);
	if (!IsValid(Player))
	{
		return;
	}
